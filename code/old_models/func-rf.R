...
runrf=function(Y,indice,lag){
  ...
  Y2 <- as.matrix(Y)
  aux <- embed(Y2, 1 + lag) # create 1 lag + forecast horizon shift (=lag option)
  
  y_train <- aux[-nrow(aux), indice] # target variable in the first column
  X_train <- aux[-nrow(aux), -indice, drop = FALSE] # predictors
  
  X.out <- aux[nrow(aux), -indice, drop = FALSE] # last row of data to be used as OOS
