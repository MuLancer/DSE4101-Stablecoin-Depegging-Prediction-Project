...
names(USDC)
str(USDC)
# timeOpen is the USDCly date -> used as dataset date
...
sum(USDC$marketCap == 0, na.rm = TRUE) 
sum(USDC$circulatingSupply == 0, na.rm = TRUE) 
# Early observations (~9-10 rows) have zero values, likely before CMC began tracking these variables
...
min(USDC$date)
max(USDC$date)
## USDC dataset covers: 2018-10-08 to 2025-12-31


# check for missing values, duplicate days, zero volume
colSums(is.na(USDC))       # none expected
sum(duplicated(USDC$date)) # none expected
sum(USDC$volume == 0)      # none expected
...
cat("1-day:  ", mean(USDC$depeg_1d, na.rm = TRUE), "\n") # ~8.5% of sample
cat("3-day:  ", mean(USDC$depeg_3d, na.rm = TRUE), "\n") # ~13.1% of sample
cat("5-day:  ", mean(USDC$depeg_5d, na.rm = TRUE), "\n") # ~15.7% of sample
cat("7-day:  ", mean(USDC$depeg_7d, na.rm = TRUE), "\n") # ~17.8% of sample

table(USDC$depeg_1d) # 222 depegs
table(USDC$depeg_3d) # 342 depegs
table(USDC$depeg_5d) # 410 depegs
table(USDC$depeg_7d) # 463 depegs
...
cat("1-day:  ", mean(USDC_final$depeg_1d, na.rm = TRUE), "\n")
cat("3-day:  ", mean(USDC_final$depeg_3d, na.rm = TRUE), "\n")
cat("5-day:  ", mean(USDC_final$depeg_5d, na.rm = TRUE), "\n")
cat("7-day:  ", mean(USDC_final$depeg_7d, na.rm = TRUE), "\n")

# Final trimmed dataset: 2606 observations (2018-11-06 to 2025-12-24)
...
# USDC coverage is similar to PAX, both starting in late 2018

# Post-trim event frequencies:
# 1-day:  8.52%  -> severe class imbalance, SMOTE will be important
# 3-day:  13.12% -> moderate imbalance
# 5-day:  15.73% -> moderate imbalance
# 7-day:  17.77% -> most balanced, but longest horizon
...
write_csv(USDC_final, "data/USDC/USDC_short_dataset.csv")
write_csv(USDC, "data/USDC/USDC_full_dataset.csv")

cat("USDC data preparation complete.\n")
cat("Final dataset dimensions:", nrow(USDC_final), "rows x", ncol(USDC_final), "cols\n")
